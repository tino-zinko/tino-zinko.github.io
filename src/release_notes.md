...
<!-- ⚠️  `about.md`にも最終更新日の記載があるため、そちらの更新を忘れないこと -->

## v048 (2026/03/02)

- 全てのコマンドの形式を、スラッシュ`/`から始まるのではなくアットマーク`@`から始まるように変更

    - 例えば、呼び名の変更は`/nickname`コマンドから`@nickname`コマンドに変更されます

    - 理由: Spoon側の仕様変更により、スラッシュ`/`から始まるコメントを入力すると、配信者の画面に届くことなくリスナーのアプリ側でエラーが発生するようになったため

## v047 (2025/11/20)

- `/ranking`コマンドにおいて、滞在時間ランキングのほか、時間ハートと粗品のランキングも表示するように修正

- `/alarm heart`コマンドと同義の`アラーム`コマンド(先頭のスラッシュ`/`は不要)を追加

    - `/alarm heart`コマンドが頻繁に使用されているため、入力しやすいショートカットを用意しました

## v046 (2025/11/09)

- `/wise`コマンドの廃止(現在はデフォルトでオンになっているため)

- `/memory`コマンドの廃止(現在はデフォルトでオンになっているため)
